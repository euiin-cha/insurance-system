...
def _parse_uploaded_file(f, co):
    """업로드된 파일 파싱 후 session_state 저장 (중복 방지)"""
    # 이미 파싱된 경우 건너뜀
    if co in st.session_state.pdf_bytes and st.session_state.pdf_bytes[co]:
        return
    with st.spinner(f"⏳ {co} PDF 파싱 중..."):
        try:
            f.seek(0)
            raw = f.read()
            if not raw:
                ...
                return
            # session_state에 즉시 저장 (rerun 전에)
            st.session_state.pdf_bytes[co] = raw
            with pdfplumber.open(io.BytesIO(raw)) as pdf:
                txt = "\n".join(p.extract_text() or "" for p in pdf.pages)
            st.session_state.pdf_texts[co] = txt
            st.session_state.pdf_structures[co] = extract_product_structure(txt, co)
            covs = extract_coverages(raw, co)
            st.session_state.pdf_coverages[co] = covs
            # ── 영속성 강화: 파싱 완료 플래그를 별도 키로 표시 ──
            st.session_state[f"_parsed_{co}"] = True
            st.success(f"✅ {co}: {len(covs)}개 담보 추출 완료")
        except Exception as e:
            # 실패 시 저장된 데이터 초기화
            ...
            st.session_state.pop(f"_parsed_{co}", None)
...
def show_upload_page():
    ...
    with st.container():
        ...
        if uf_base:
            ...
            if base_co_up in st.session_state.pdf_bytes:
                st.session_state.pdf_bytes.pop(base_co_up, None)
                st.session_state.pdf_coverages.pop(base_co_up, None)
                st.session_state.pdf_structures.pop(base_co_up, None)
                st.session_state.pdf_texts.pop(base_co_up, None)
                st.session_state.pop(f"_parsed_{base_co_up}", None)
            _parse_uploaded_file(uf_base, base_co_up)
    ...
    for row_start in range(0, len(others), 3):
        ...
        for ci, co in enumerate(others[row_start:row_start+3]):
            with cols[ci]:
                ...
                if uf:
                    if co in st.session_state.pdf_bytes:
                        st.session_state.pdf_bytes.pop(co, None)
                        st.session_state.pdf_coverages.pop(co, None)
                        st.session_state.pdf_structures.pop(co, None)
                        st.session_state.pdf_texts.pop(co, None)
                        st.session_state.pop(f"_parsed_{co}", None)
                    _parse_uploaded_file(uf, co)
            ...
            with st.spinner("🤖 손해보험 전문가 AI가 보고서를 작성 중입니다..."):
                try:
                    try:
                        ...
                        st.error(
                            f"❌ 필수 패키지가 설치되지 않았습니다: {ie}\n\n"
                            "터미널에서 아래 명령어를 실행 후 Streamlit을 재시작하세요:\n\n"
                            "```\npip install anthropic httpx\n```"
                        )
                        st.stop()
                    # ── 타임아웃 300초 설정 (502 방지 핵심) ──────────
                    ...
                    with client.messages.stream(
                        ...
                        for chunk in stream.text_stream:
                            raw_text += chunk
                            # 실시간 진행 표시 (선택적)
                            if len(raw_text) % 500 < 20:
                                status_placeholder.caption(f"⏳ 보고서 생성 중... ({len(raw_text)}자 수신)")
                    ...
                    err_msg = str(e)
                    if "Connection" in err_msg or "connect" in err_msg.lower() or "502" in err_msg:
                        st.error(
                            "❌ API 연결 오류 (502/타임아웃)\n\n"
                            "**해결 방법:**\n"
                            "1. 환경변수 `ANTHROPIC_API_KEY` 설정 확인\n"
                            "2. `pip install anthropic httpx` 재설치\n"
                            "3. 네트워크/방화벽에서 `api.anthropic.com` 허용 여부 확인\n\n"
                            f"상세 오류: {err_msg}"
                        )
                    else:
