...
for key in ['pdf_bytes','pdf_texts','pdf_structures','pdf_coverages',
            'tab1_results','tab2_results','tab3_report']:
...
def show_upload_page():
    ...
    with st.container():
        st.markdown(f'<div style="{card_style}"><span style="font-size:15px;font-weight:600">{dot_base} {base_co_up}</span>{badge_html}</div>',
                    unsafe_allow_html=True)
        fname_base = list(st.session_state.pdf_bytes.keys())
        status_base = f"✅ 업로드 완료" if is_uploaded_base else "파일을 추가하세요"
        st.markdown(f'<div style="color:{"#2E7D32" if is_uploaded_base else "#999"};font-size:12px;margin:-12px 0 8px 4px">{status_base}</div>',
                    unsafe_allow_html=True)
        ...
        for ci, co in enumerate(others[row_start:row_start+3]):
            with cols[ci]:
                ...
                st.markdown(f'<div style="{c_style}"><span style="font-size:13px;font-weight:600">{dot} {co}</span></div>',
                            unsafe_allow_html=True)
                status = "✅ 업로드 완료" if is_up else "파일을 추가하세요"
                st.markdown(f'<div style="color:{"#2E7D32" if is_up else "#999"};font-size:11px;margin:4px 0 6px 2px">{status}</div>',
                            unsafe_allow_html=True)
...
def show_tab1_content():
    ...
    if not companies_loaded_fn:
        st.info("👈 파일 업로드 메뉴에서 PDF를 먼저 업로드하세요.")
        return
                ...
                try:
                    ...
                    if "Connection" in err_msg or "connect" in err_msg.lower():
                        st.error(
                            "❌ API 연결 오류\n\n"
                            "**해결 방법:**\n"
                            "1. `pip install anthropic` 실행 후 재시작\n"
                            "2. API 키 확인: 환경변수 ANTHROPIC_API_KEY 설정 여부\n"
                            "3. 인터넷 연결 확인\n\n"
                            f"상세 오류: {err_msg}"
                        )
                    else:
...
def render_report_html(report_data, base_co, comp_cos, struct_data=None):
    ...
    struct_gap_section_inner = struct_gap_section
